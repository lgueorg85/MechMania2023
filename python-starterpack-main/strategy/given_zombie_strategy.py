...
class GivenZombieStrategy(Strategy):
    ...
    def decide_moves(
        ...
        if(game_state.turn < 6):
            for [character_id, moves] in possible_moves.items():
                ...
                # Only move zombies
                if game_state.characters[character_id].is_zombie:
                    # Choose the move action that moves straight down (assuming y-axis points upwards)
                    move_choice = moves[0]
                    for m in moves:
                        if m.destination.y > move_choice.destination.y:
                            move_choice = m
